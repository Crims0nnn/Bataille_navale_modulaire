...
int main()
{
    ...
    do {
        ...
        if (etatFlot(flotteJoueur1) != aSombrer) {
            ...
            if (controlFlotte(flotteJoueur2, mCase) == Bool::True) {
                printf("Tir ok\n");
        ...
        if (etatFlot(flotteJoueur2) != aSombrer) {
            ...
            if (controlFlotte(flotteJoueur1, mCase) == Bool::True) {
                printf("Tir ok\n");
...
Bateau createBateau(Case nCase, int taille) {
    ...
    switch (random) {
    case 0: positionBateau::enLigne; break;
    case 1: positionBateau::enColonne; break;
    case 2: positionBateau::enDiag; break;
    };
    //positionBateau 
    ...
    for (i = 0; i < MAXCASE; i++) {
        ...
        if (positionBateau == positionBateau::enLigne) {

            nCase.col++;

        }
        else if (positionBateau == positionBateau::enColonne) {

            nCase.ligne++;

        }
        else if (positionBateau == positionBateau::enDiag) {
...
Bool controlCase(Bateau mBat, Case mCase) {


    int i;
    Bool valTest = Bool::False;
    for (i = 0; i < MAXCASE; i++) {

        if (cmpCase(mBat.nCase[i], mCase) == Bool::True)
            valTest = Bool::True;
...
//return True si la case passé en paramètre appatient bien à un bateau d'une flotte
 Bool controlFlotte(Flotte f, Case mCase) {

    int i;
  Bool valTest = Bool::False;
    for (i = 0; i < NBBATEAU; i++) {
        if (controlCase(f.nBateau[i], mCase) == Bool::True)
            valTest = Bool::True;
...
Bool cmpCase(Case mCase, Case tCase) {
